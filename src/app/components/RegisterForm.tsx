...
export default function RegisterForm({}: Props) {
  ...
  const emailhandle = useCallback((event : React.ChangeEvent<HTMLInputElement>)=>{
      const email = event.target.value
      setemailValid(/^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,4}$/.test(email))
  },[])
  
  const passwordhandle = useCallback((event : React.ChangeEvent<HTMLInputElement>)=>{
      const pwd  = event.target.value
      setPasswordvalid(pwd.length >= 8 )
  },[])
  const resswordhandle = useCallback((event : React.ChangeEvent<HTMLInputElement>)=>{
